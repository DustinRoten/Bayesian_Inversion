...
if(file.exists('X_config.RData')) {
  ...
  flux_times <- seq(from = ISOdatetime(y1, m1, d1, h1, mn1, 0, tz = "UTC"),
...
if (ntimes == 1) D <- as.matrix(D)  #make sure D is in ntime x ntime format even if ntime = 1
...
if (time_span == 1) S <- matrix(S, nrow = time_span, byrow = T)  #ensure that S is in matrix-conformable format, 1 x ncells
...
prior_tot_unc <- W %*% (Qsum/k2) %*% t(W)
post_tot_unc <- W %*% (Vshat_bar) %*% t(W)
unc_red <- prior_tot_unc - post_tot_unc

#TEMP DELETE ME!!!
if(post_tot_unc < 0) write.table('Negative Uncertainty', file = 'out/WARNING.txt', append = FALSE)
...
if(length(isubset) > 1){
  ...
  # multiply uncertainty covariance matrixes by the aggregation operators for
  # covariance-included total uncertainties
  prior_subset_unc <- W %*% (Qsum_subset/k2_subset) %*% t(W)
  post_subset_unc <- W %*% (Vshat_bar_subset) %*% t(W)
